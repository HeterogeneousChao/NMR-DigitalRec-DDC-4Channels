...
%         eval(str_tmp);
 ...
 %%%% generate fpga config file
    ...
    if ((cic_decf*cic_diffd) > 1)
        for idx = 1 : (cic_numsecs) - 1
            cic_spec = filter(cic_spec, 1, delta);
        end
    end
 ...
 %% %%%%%%%%%%%%%%%% plot %%%%%%%%%%%%%%%%% 
    ...
    subplot(3, 1, 3);
    plot(cic_f, pi*(cic_spec_ang/180), 'k');
    xlabel('frequency (*fs) Hz)'); ylabel('Angle (pi)'); grid on; 
    title('CIC');
...
plot(f_cic, cic_out_mag, 'k');
axis auto;
%axis([0 1 -inf inf]);
xlabel('frequency (*fs/(cic-decf*cic-diffd) Hz)'); ylabel('Magnitude (dB)'); grid on;grid on;grid on;
title('After CIC filter');

subplot(3, 1, 3);
plot(f_cic, pi*(cic_out_ang/180), 'k');
xlabel('frequency (*fs/(cic-decf*cic-diffd) Hz)'); ylabel('Angle (pi)'); grid on; 
title('After CIC filter');
